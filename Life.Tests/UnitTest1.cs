...
namespace LifeTests
{
    public class BoardTests
    {
        ...
        // 2. Randomize создаёт живые клетки
        [Fact]
        ...
        // 5. Мигалка (blinker) имеет период 2
        [Fact]
        ...
        // 6. Поиск связных компонент (один блок)
        [Fact]
        ...
        // 10. Сохранение и загрузка состояния
        [Fact]
        ...
        // 11. Стабильное поколение (уже стабильное)
        [Fact]
        public void FindStableGeneration_ReturnsEarlyForStable()
        {
            var board = new Board(6, 6);
            board.LoadPattern(new[] { (0,0),(1,0),(0,1),(1,1) }); // блок
            int gen = board.FindStableGeneration(5, 100);
            Assert.InRange(gen, 0, 5); // стабильность с первого поколения
        }

        // 12. Пустое поле остаётся пустым
        [Fact]
        ...
        // 13. Правило HighLife (B36/S23) не вызывает ошибок
        [Fact]
        public void RuleParsing_AllowsOtherRules()
        {
            ...
            board.Randomize(0.3);
            board.NextGeneration();
            Assert.True(true); // главное – отсутствие исключений
        }

        // 14. Сериализация/десериализация настроек
        [Fact]
        ...
        // 15. Несколько несвязных компонент
        [Fact]
        ...
        // Дополнительный тест: классификация лодки (boat)
        [Fact]
